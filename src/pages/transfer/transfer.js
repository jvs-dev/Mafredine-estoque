...
import { getFirestore, collection, addDoc, doc, setDoc, onSnapshot, query, where, updateDoc, arrayUnion, arrayRemove, getDoc, getDocs, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            let usersdocref = onSnapshot(doc(db, `users`, `${user.email}`), (doc) => {
                ...
                SendSearchInput.addEventListener("input", (evt) => {
                    if (evt.target.value != "") {
                        let SectionItemsCardsSend = document.getElementById("SectionItemsCardsSend")
                        SectionItemsCardsSend.innerHTML = ""
