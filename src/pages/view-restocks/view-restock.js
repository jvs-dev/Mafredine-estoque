...
function verifyUrl() {
    ...
    if (dataId != null) {
        let unsub = onSnapshot(doc(db, "restock", `${dataId}`), (doc) => {
            ...
            <div class="content">
                <h2 class="viewDischargeSection__h2">Entrada de itens</h2>
            ...
            generatePDF.onclick = function () {
                let doc = new jsPDF()
                doc.fromHTML(`<h1 style="font-size: 32px; font-weight: 500; font-family: 'Poppins', sans-serif;">Comprovante de entrada de itens</h1>`, 10, 10)
                ...
                doc.fromHTML(`<p style="font-size: 18px; font-family: 'Poppins', sans-serif;">Adição feita por ${recorderName}.</p>`, 10, 48)
                doc.fromHTML(`<p style="font-size: 18px; font-family: 'Poppins', sans-serif;">Adição feita dia ${requestDate} ás ${requesthour}.</p>`, 10, 56)
                doc.fromHTML(`<p style="font-size: 18px; font-family: 'Poppins', sans-serif;">Descrição da adição: ${requestDescription}.</p>`, 10, 64)
                doc.fromHTML(`<p style="font-size: 18px; font-family: 'Poppins', sans-serif;">Itens adicionados:</p>`, 10, 72)
                doc.fromHTML(`<ul style="font-size: 18px; font-family: 'Poppins', sans-serif;"><br>${itemsToTransferList}${actualUserWork != "Técnico" ? `<p style="font-size: 18px; font-family: 'Poppins', sans-serif;">Total adicionado: <span style="font-weight: 500;">$${returnTotalValue()}</span>.</p>` : ""}</ul>`, 10, 80)
